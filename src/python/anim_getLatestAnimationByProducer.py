...
if len(sys.argv) != 3:
  print("USAGE: python anim_getLatestAnimationByProducer.py <producerName>[,<producerName>] <outputDir>")
  sys.exit(-1)
...
for t in timeList:
  ...
  for c in contentList:
    ...
    if i == 0:  
      ...
      print("python3 %s/graph_getCoverBorders.py %s %s 240 \"#FFFFFF\" 1.0 %s %d %d" % (pdir,c.fileId,c.messageIndex,fname_landBorder1,width,height))
      print("python3 %s/graph_getCoverBorders.py %s %s 240 \"#000000\" 0.0 %s %d %d" % (pdir,c.fileId,c.messageIndex,fname_landBorder2,width,height))
      
    if t == c.forecastTime  and  c.geometryId == geometryId:

      if c.fmiParameterName == "T-K" and c.fmiParameterLevelId == 6 and c.parameterLevel == 2:
        file_T_K = c.fileId
        message_T_K = c.messageIndex
      
      if c.fmiParameterName == "U-MS" and c.fmiParameterLevelId == 6 and c.parameterLevel == 10:
        file_U_MS = c.fileId
        message_U_MS = c.messageIndex

      if c.fmiParameterName == "V-MS" and c.fmiParameterLevelId == 6 and c.parameterLevel == 10:
        file_V_MS = c.fileId
  ...
  if file_T_K > 0:
    print("python3 %s/graph_getGrid.py %d %d 0 1.0 jet 1 0 1.0 -0.01 0.01 %s %d %d" % (pdir,file_T_K,message_T_K,fname_T_K_isobands,width,height))
    print("python3 %s/graph_getNumbers.py %d %d -273.15 1.0 17 10 \"000000\" 1 0 \"{:0.0f}\" %s %d %d" % (pdir,file_T_K,message_T_K,fname_T_K_numbers,width,height))
    
  if file_U_MS > 0  and file_V_MS > 0:
    print("python3 %s/graph_getArrows.py %d %d  %s %s 10 \"#8080A0\" 0.7 %s %d %d" % (pdir,file_U_MS,message_U_MS,file_V_MS,message_V_MS,fname_wind_arrows,width,height))  

  if file_P_PA > 0:    
    print("python3 %s/graph_getIsolines.py %d %d 40 \"#000040\" 0.4 %s %d %d" % (pdir,file_P_PA,message_P_PA,fname_P_PA_isolines,width,height))
    
  if file_T_K > 0 and  file_U_MS > 0  and file_V_MS > 0 and file_P_PA > 0:
    print("%s/gu_mergePngFiles %s/%s_%d_merge_%04d.png FFE0E0FF %s %s %s %s %s %s" % (mdir,outDir,producers[0],geometryId,ti,fname_T_K_isobands,fname_P_PA_isolines,fname_wind_arrows,fname_landBorder1,fname_landBorder2,fname_T_K_numbers))
