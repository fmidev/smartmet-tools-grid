/*! \file
 *  \brief Content Server client: queries and prints a single content record identified by fileId and messageIndex from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getContentInfo <sessionId> <fileId> <messageIndex> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentInfo(sessionId,fileId,messageIndex,info);
