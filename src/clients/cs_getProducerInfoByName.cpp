/*! \file
 *  \brief Content Server client: retrieves producer metadata for a single producer identified by its name.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getProducerInfoByName <sessionId> <producerName> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getProducerInfoByName(sessionId,producerName,info);
