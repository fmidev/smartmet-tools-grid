...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoList <sessionId> <startFileId> <maxRecords> [-http <url>]\n");
      return -1;
    }
    ...
    if (argc == 6  &&  strcmp(argv[4],"-http") == 0)
    {
      ...
      startTime = getTime();
      result = service.getFileInfoList(sessionId,startFileId,maxRecords,infoList);
      endTime = getTime();
    }
    else
    {
