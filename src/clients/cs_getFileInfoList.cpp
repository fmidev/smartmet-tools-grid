/*! \file
 *  \brief Content Server client: retrieves a paginated list of all registered files starting from a given file ID, up to a maximum record count.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoList <sessionId> <startFileId> <maxRecords> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
