/*! \file
 *  \brief Content Server client: deletes all content records belonging to the producer identified by name from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteContentListByProducerName <sessionId> <producerName> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteContentListByProducerName(sessionId,producerName);
