/*! \file
 *  \brief Content Server client: retrieves the full list of all generation records from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 2)
    {
      fprintf(stdout,"USAGE: cs_getGenerationInfoList <sessionId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getGenerationInfoList(sessionId,infoList);
