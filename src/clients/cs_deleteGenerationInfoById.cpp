/*! \file
 *  \brief Content Server client: deletes the generation record identified by generationId from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteGenerationInfoById <sessionId> <generationId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteGenerationInfoById(sessionId,generationId);
