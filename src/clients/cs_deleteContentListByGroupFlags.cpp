...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteContentListByGroupFlags <sessionId> <groupFlags> [[-http <url>]|[-redis <address> <port> <tablePrefix>]]\n");
      return -1;
    }
    ...
    int result = service->deleteContentListByGroupFlags(sessionId,groupFlags);
