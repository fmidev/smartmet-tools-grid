/*! \file
 *  \brief Content Server client: retrieves a paginated list of files belonging to a given source ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoListBySourceId <sessionId> <sourceId> <startFileId> <maxRecords> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getFileInfoListBySourceId(sessionId,sourceId,startFileId,maxRecords,infoList);
