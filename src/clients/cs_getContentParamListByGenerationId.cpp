/*! \file
 *  \brief Content Server client: retrieve the full content parameter list for a given generation ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getContentParamListByGenerationId <sessionId> <generationId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentParamListByGenerationId(sessionId,generationId,contentParamList);
    ...
    // ### Result:
    contentParamList.writeToFile("/dev/stdout");
