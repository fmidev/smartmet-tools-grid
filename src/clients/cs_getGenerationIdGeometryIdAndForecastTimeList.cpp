/*! \file
 *  \brief Content Server client: retrieves a list of unique (generationId, geometryId, forecastTime) tuples from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 2)
    {
      fprintf(stdout,"USAGE: cs_getGenerationIdGeometryIdAndForecastTimeList <sessionId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getGenerationIdGeometryIdAndForecastTimeList(sessionId,infoList);
