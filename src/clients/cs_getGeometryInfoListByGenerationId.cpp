/*! \file
 *  \brief Content Server client: retrieves all geometry records associated with a given generation ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getGeometryInfoListByGenerationId <sessionId> <generationId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    T::GenerationId generationId = toUInt64(argv[2]);
    ...
    int result = service->getGeometryInfoListByGenerationId(sessionId,generationId,infoList);
