/*! \file
 *  \brief Content Server client: retrieves all geometry records associated with a given source ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getGeometryInfoListBySourceId <sessionId> <sourceId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    T::SourceId sourceId = toUInt32(argv[2]);
    ...
    int result = service->getGeometryInfoListBySourceId(sessionId,sourceId,infoList);
