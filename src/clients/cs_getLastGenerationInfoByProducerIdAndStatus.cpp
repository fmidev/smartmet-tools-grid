/*! \file
 *  \brief Content Server client: retrieves the most recent generation matching a given producer ID and generation status.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getLastGenerationInfoByProducerIdAndStatus <sessionId> <producerId> <generationStatus> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    uchar generationStatus = toInt64(argv[3]);
    ...
    int result = service->getLastGenerationInfoByProducerIdAndStatus(sessionId,producerId,generationStatus,info);
