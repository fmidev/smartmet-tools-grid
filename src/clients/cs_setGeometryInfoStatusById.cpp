/*! \file
 *  \brief Content Server client: sets the status of a geometry record identified by generation ID, geometry ID, and level ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 6)
    {
      fprintf(stdout,"USAGE: cs_setGeometryInfoStatusById <sessionId> <generationId> <geometryId> <levelId> <status> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->setGeometryInfoStatusById(sessionId,generationId,geometryId,levelId,status);
