/*! \file
 *  \brief Content Server client: retrieves all producer records belonging to a given source ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getProducerInfoListBySource <sessionId> <sourceId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    T::SourceId sourceId = atol(argv[2]);
    ...
    int result = service->getProducerInfoListBySourceId(sessionId,sourceId,infoList);
