/*! \file
 *  \brief Content Server client: retrieves geometry metadata identified by generation ID, geometry ID, and level ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getGeometryInfoById <sessionId> <generationId> <geometryId> <levelId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
