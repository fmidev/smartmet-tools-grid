/*! \file
 *  \brief Content Server client: registers a new generation (forecast run) record (producerId, type, sourceId, flags, name, description, analysisTime, status) in the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 10)
    {
      fprintf(stdout,"USAGE: cs_addGenerationInfo <sessionId> <producerId> <generationType> <sourceId> <flags> <name> <description> <analysisTime> <status> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>][-ior <contentServerIor>]]\n");
      return -1;
    }
    ...
    int result = service->addGenerationInfo(sessionId,info);
