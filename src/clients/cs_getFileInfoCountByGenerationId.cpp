/*! \file
 *  \brief Content Server client: retrieves the number of registered files belonging to a given generation ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoCountByGenerationId <sessionId> <generationId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getFileInfoCountByGenerationId(sessionId,generationId,count);
