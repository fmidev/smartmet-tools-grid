/*! \file
 *  \brief Content Server client: retrieves a paginated list of files belonging to a given producer ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoListByProducerId <sessionId> <producerId> <startFileId> <maxRecords> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getFileInfoListByProducerId(sessionId,producerId,startFileId,maxRecords,infoList);
