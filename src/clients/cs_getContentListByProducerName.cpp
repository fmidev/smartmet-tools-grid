/*! \file
 *  \brief Content Server client: retrieve content list for a given producer name.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 6)
    {
      fprintf(stdout,"USAGE: cs_getContentListByProducerName <sessionId> <producerName> <startFileId> <startMessageIndex> <maxRecords> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentListByProducerName(sessionId,producerName,startFileId,startMessageIndex,maxRecords,infoList);
