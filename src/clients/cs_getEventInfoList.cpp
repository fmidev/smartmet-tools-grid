/*! \file
 *  \brief Content Server client: retrieves a list of events starting from a given event ID, up to a maximum record count.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getEventInfoList <sessionId> <startEventId> <maxRecords> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    T::EventInfoList infoList;
    uint startEventId = toInt64(argv[2]);
    ...
    int result = service->getEventInfoList(sessionId,0,startEventId,maxRecords,infoList);
