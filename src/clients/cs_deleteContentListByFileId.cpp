/*! \file
 *  \brief Content Server client: deletes all content records associated with the given fileId from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteContentListByFileId <sessionId> <fileId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteContentListByFileId(sessionId,fileId);
