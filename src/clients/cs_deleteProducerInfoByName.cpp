/*! \file
 *  \brief Content Server client: deletes the producer record identified by name from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteProducerInfoByName <sessionId> <producerName> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteProducerInfoByName(sessionId,producerName);
