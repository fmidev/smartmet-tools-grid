/*! \file
 *  \brief Content Server client: deletes all generation records belonging to the given sourceId from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteGenerationInfoListBySourceId <sessionId> <sourceId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteGenerationInfoListBySourceId(sessionId,sourceId);
