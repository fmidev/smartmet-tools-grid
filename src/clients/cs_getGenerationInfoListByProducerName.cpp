/*! \file
 *  \brief Content Server client: retrieves all generation records belonging to a producer identified by name.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getGenerationInfoListByProducerName <sessionId> <producerName> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getGenerationInfoListByProducerName(sessionId,producerName,infoList);
