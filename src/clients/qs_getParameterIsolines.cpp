...
int main(int argc, char *argv[])
{
  try
  {
    ...
    if (argc < 11)
    {
      fprintf(stdout,"USAGE: qs_getParameterIsolines <sessionId> <parameter> <gridGeometryId> <multiplyer> <rotate> <startTime> <timeStepSizeInMinutes> <timesteps> <filePrefix> <contourVal1> [..<contourValN>]\n");
      return -1;
    }
    ...
    for (uint t=0; t<timesteps; t++)
    {
      for (auto it = queryList[t].mQueryParameterList.begin(); it != queryList[t].mQueryParameterList.end(); ++it)
      {
        for (auto v = it->mValueList.begin(); v != it->mValueList.end(); ++v)
        {
          ...
          paintWkb(image,imageWidth,imageHeight,false,rotate,1,0,0,v->mWkbList,0x00);
