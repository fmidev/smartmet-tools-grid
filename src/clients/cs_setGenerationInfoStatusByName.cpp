/*! \file
 *  \brief Content Server client: sets the status of a generation identified by its name.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_setGenerationInfoStatusByName <sessionId> <generationName> <status> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    std::string generationName = argv[2];
    uchar status = (uchar)toInt64(argv[3]);
    ...
    int result = service->setGenerationInfoStatusByName(sessionId,generationName,status);
