/*! \file
 *  \brief Content Server client: registers a new producer record (producerId, sourceId, flags, status, name, title, description) in the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 9)
    {
      fprintf(stdout,"USAGE: cs_addProducerInfo <sessionId> <producerId> <sourceId> <flags> <status> <name> <title> <descr>  [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->addProducerInfo(sessionId,info);
