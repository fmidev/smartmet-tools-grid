/*! \file
 *  \brief Content Server client: queries and prints a paginated list of content records for the given generationId, starting at a specified fileId and messageIndex.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 7)
    {
      fprintf(stdout,"USAGE: cs_getContentListByGenerationId <sessionId> <generationId> <startFileId> <startMessageIndex> <maxRecords> <requestFlags> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    T::FileId startFileId = toInt64(argv[3]);
    uint startMessageIndex = toInt64(argv[4]);
    uint maxRecords = toInt64(argv[5]);
    uint requestFlags = toInt64(argv[6]);
    ...
    int result = service->getContentListByGenerationId(sessionId,generationId,startFileId,startMessageIndex,maxRecords,requestFlags,infoList);
