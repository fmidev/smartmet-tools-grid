...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getDataServerInfoById <sessionId> <serverId> [[-http <url>]|[-redis <address> <port> <tablePrefix>]]\n");
      return -1;
    }
    ...
    if (strcmp(argv[argc-2],"-http") == 0)
    {
      ...
      startTime = getTime();
      result = service.getDataServerInfoById(sessionId,serverId,info);
      endTime = getTime();
    }
    else
