/*! \file
 *  \brief Content Server client: retrieve content time list for a given generation, geometry, and level ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getContentTimeListByGenerationGeometryAndLevelId <sessionId> <generationId> <geometryId> <levelId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentTimeListByGenerationGeometryAndLevelId(sessionId,generationId,geometryId,levelId,timeList);
