/*! \file
 *  \brief Content Server client: retrieves file metadata for a specified list of file IDs.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout, "USAGE: cs_getFileInfoListByFileIdList <sessionId> <fileId> [<fileId> .. <fileId>] [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getFileInfoListByFileIdList(sessionId,fileIdList,infoList);
