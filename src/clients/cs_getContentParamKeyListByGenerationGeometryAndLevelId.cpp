/*! \file
 *  \brief Content Server client: retrieve parameter key list for a given generation, geometry, and level ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getContentParamListByGenerationGeometryAndLevelId <sessionId> <generationId> <geometryId> <levelId> <paramKeyType> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
