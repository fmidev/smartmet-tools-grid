/*! \file
 *  \brief Content Server client: retrieve content time list for a given generation and geometry ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getContentTimeListByGenerationAndGeometryId <sessionId> <generationId> <geometryId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentTimeListByGenerationAndGeometryId(sessionId,generationId,geometryId,timeList);
