/*! \file
 *  \brief Content Server client: retrieves the parameter list for a given producer ID, converting between source and target parameter key types.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getProducerParameterListByProducerId <sessionId> <producerId> <sourceParameterKeyType> <targetParameterKeyType> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
