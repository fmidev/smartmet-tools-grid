/*! \file
 *  \brief Content Server client: retrieves the total number of generations registered in the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 2)
    {
      fprintf(stdout,"USAGE: cs_getGenerationInfoCount <sessionId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getGenerationInfoCount(sessionId,count);
