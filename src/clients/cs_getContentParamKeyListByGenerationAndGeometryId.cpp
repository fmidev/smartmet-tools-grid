/*! \file
 *  \brief Content Server client: retrieve parameter key list for a given generation and geometry ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getContentParamListByGenerationAndGeometryId <sessionId> <generationId> <geometryId> <paramKeyType> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    T::ParamKeyType paramKeyType = toInt64(argv[4]);
    ...
    int result = service->getContentParamKeyListByGenerationAndGeometryId(sessionId,generationId,geometryId,paramKeyType,paramKeyList);
