/*! \file
 *  \brief Content Server client: queries and prints the list of parameter levels for a given generationId, geometryId, and levelId from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getContentLevelListByGenerationGeometryAndLevelId <sessionId> <generationId> <geometryId> <levelId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    short levelId = toInt64(argv[4]);
    std::set<T::ParamLevel> levelList;
    ...
    int result = service->getContentLevelListByGenerationGeometryAndLevelId(sessionId,generationId,geometryId,levelId,levelList);
