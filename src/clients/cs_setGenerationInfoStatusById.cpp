/*! \file
 *  \brief Content Server client: sets the status of a generation identified by its numeric generation ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_setGenerationInfoStatusById <sessionId> <generationId> <status> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    T::GenerationId generationId = toInt64(argv[2]);
    uchar status = toInt64(argv[3]);
    ...
    int result = service->setGenerationInfoStatusById(sessionId,generationId,status);
