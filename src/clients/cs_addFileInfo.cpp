/*! \file
 *  \brief Content Server client: registers a single grid file (producerId, generationId, fileType, sourceId, flags, filename) in the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 8)
    {
      fprintf(stdout,"USAGE: cs_addFileInfo <sessionId> <producerId> <generationId> <fileType> <sourceId> <flags> <filename> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
