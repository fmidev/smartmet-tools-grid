/*! \file
 *  \brief Content Server client: retrieves file metadata for a single file identified by its filename.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoById <sessionId> <filename> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getFileInfoByName(sessionId,fileName,info);
