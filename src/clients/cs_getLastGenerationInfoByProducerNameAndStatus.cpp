/*! \file
 *  \brief Content Server client: retrieves the most recent generation matching a given producer name and generation status.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getLastGenerationInfoByProducerNameAndStatus <sessionId> <producerName> <generationStatus> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    uchar generationStatus = (uchar)toInt64(argv[3]);
    ...
    int result = service->getLastGenerationInfoByProducerNameAndStatus(sessionId,producerName,generationStatus,info);
