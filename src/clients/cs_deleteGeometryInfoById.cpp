/*! \file
 *  \brief Content Server client: deletes the geometry info record identified by generationId, geometryId, and levelId from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_setGeometryInfoStatusById <sessionId> <generationId> <geometryId> <levelId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteGeometryInfoById(sessionId,generationId,geometryId,levelId);
