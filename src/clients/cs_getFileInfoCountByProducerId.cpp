/*! \file
 *  \brief Content Server client: retrieves the number of registered files belonging to a given producer ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoCountByProducerId <sessionId> <producerId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
