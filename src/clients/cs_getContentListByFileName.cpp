/*! \file
 *  \brief Content Server client: queries and prints all content records associated with the given filename from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getContentListByFileName <sessionId> <filename> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentListByFileName(sessionId,filename,infoList);
