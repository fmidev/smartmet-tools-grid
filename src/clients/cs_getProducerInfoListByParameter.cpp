...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 4)
    {
      fprintf(stdout,"USAGE: cs_getProducerInfoListBySource <sessionId> <paramKeyType> <paramKey> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    T::ParamKeyType paramKeyType = atol(argv[2]);
    ...
    int result = service->getProducerInfoListByParameter(sessionId,paramKeyType,paramKey,infoList);
