/*! \file
 *  \brief Content Server client: queries and prints content records for a given generationId whose forecast time falls within the specified startTime/endTime range.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getContentListByGenerationIdAndTimeRange <sessionId> <generationId> <startTime> <endTime> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentListByGenerationIdAndTimeRange(sessionId,generationId,startT,endT,infoList);
