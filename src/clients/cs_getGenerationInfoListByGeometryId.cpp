/*! \file
 *  \brief Content Server client: retrieves all generation records that use a given geometry ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getGenerationInfoListByGeometryId <sessionId> <geometryId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    uint geometryId = toInt64(argv[2]);
    ...
    int result = service->getGenerationInfoListByGeometryId(sessionId,geometryId,infoList);
