...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteFileInfoListByGroupFlags <sessionId> <groupFlagsId> [[-http <url>]|[-redis <address> <port> <tablePrefix>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    uint groupFlagsId = toInt64(argv[2]);
    ...
    int result = service->deleteFileInfoListByGroupFlags(sessionId,groupFlagsId);
