/*! \file
 *  \brief Content Server client: retrieves generation metadata for a single generation identified by its name.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getGenerationInfoByName <sessionId> <generationName> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getGenerationInfoByName(sessionId,generationName,info);
