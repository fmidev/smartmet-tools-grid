...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_unregisterContentListByServerId <sessionId> <serverId> [[-http <url>]|[-redis <address> <port> <tablePrefix>]]\n");
      return -1;
    }
    ...
    int result = service->unregisterContentList(sessionId,serverId);
