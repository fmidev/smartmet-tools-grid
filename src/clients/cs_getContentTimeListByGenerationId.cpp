/*! \file
 *  \brief Content Server client: retrieve content time list for a given generation ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getContentTimeListByGenerationId <sessionId> <generationId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentTimeListByGenerationId(sessionId,generationId,timeList);
