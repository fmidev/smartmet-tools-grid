/*! \file
 *  \brief Content Server client: deletes file registration records for a given generationId filtered by geometryId, forecastType, perturbation number, and forecastTime from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 7)
    {
      fprintf(stdout,"USAGE: cs_deleteFileInfoListByGenerationIdAndForecastTime <sessionId> <generationId> <geometryId> <forecastType> <forecastPertubationNumber> <forecastTime> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
