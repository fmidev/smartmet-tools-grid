/*! \file
 *  \brief Content Server client: deletes all geometry info records belonging to the given generationId from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteGeometryInfoListByGenerationId <sessionId> <generationId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteGeometryInfoListByGenerationId(sessionId,generationId);
