...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 6)
    {
      fprintf(stdout,"USAGE: cs_getContentListByServerId <sessionId> <serverId> <startFileId> <startMessageIndex> <maxRecords> [[-http <url>]|[-redis <address> <port> <tablePrefix>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    T::ContentInfoList infoList;
    uint serverId = toInt64(argv[2]);
    uint startFileId = toInt64(argv[3]);
    ...
    int result = service->getContentListByServerId(sessionId,serverId,startFileId,startMessageIndex,maxRecords,infoList);
