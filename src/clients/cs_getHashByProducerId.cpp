/*! \file
 *  \brief Content Server client: retrieves a content hash value for all data associated with a given producer ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getHashByProducerId <sessionId> <producerId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
