/*! \file
 *  \brief Content Server client: queries and prints the total number of content records stored in the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 2)
    {
      fprintf(stdout,"USAGE: cs_getContentCount <sessionId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentCount(sessionId,count);
