/*! \file
 *  \brief Content Server client: retrieves the number of registered files belonging to a given source ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoCountBySourceId <sessionId> <sourceId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getFileInfoCountBySourceId(sessionId,sourceId,count);
