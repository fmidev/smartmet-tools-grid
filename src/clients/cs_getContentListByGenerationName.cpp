...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 6)
    {
      fprintf(stdout,"USAGE: cs_getContentListByGenerationName <sessionId> <generationName> <startFileId> <startMessageIndex> <maxRecords> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getContentListByGenerationName(sessionId,generationName,startFileId,startMessageIndex,maxRecords,infoList);
