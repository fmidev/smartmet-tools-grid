/*! \file
 *  \brief Content Server client: retrieves a paginated list of files belonging to a producer identified by name.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 5)
    {
      fprintf(stdout,"USAGE: cs_getFileInfoListByProducerName <sessionId> <producerName> <startFileId> <maxRecords> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getFileInfoListByProducerName(sessionId,producerName,startFileId,maxRecords,infoList);
