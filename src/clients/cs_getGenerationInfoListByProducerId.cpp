/*! \file
 *  \brief Content Server client: retrieves all generation records belonging to a given producer ID.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getGenerationInfoListByProducerId <sessionId> <producerId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->getGenerationInfoListByProducerId(sessionId,producerId,infoList);
