/*! \file
 *  \brief Content Server client: deletes all geometry info records belonging to the given producerId from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteGeometryInfoListByProducerId <sessionId> <producerId> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteGeometryInfoListByProducerId(sessionId,producerId);
