...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_getContentListByRequestCounterKey <sessionId> <requestCounterKey> [[-http <url>]|[-redis <address> <port> <tablePrefix>]]\n");
      return -1;
    }

    T::SessionId sessionId = toInt64(argv[1]);
    T::ContentInfoList infoList;
    ulonglong key = toInt64(argv[2]);
    ...
    int result = service->getContentListByRequestCounterKey(sessionId,key,infoList);
