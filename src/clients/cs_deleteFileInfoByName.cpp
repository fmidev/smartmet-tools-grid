/*! \file
 *  \brief Content Server client: deletes the file registration record identified by filename from the Content Server.
 */
...
int main(int argc, char *argv[])
{
  try
  {
    if (argc < 3)
    {
      fprintf(stdout,"USAGE: cs_deleteFileInfoById <sessionId> <filename> [[-http <url>]|[-redis <address> <port> <tablePrefix> <password>]]\n");
      return -1;
    }
    ...
    int result = service->deleteFileInfoByName(sessionId,filename);
